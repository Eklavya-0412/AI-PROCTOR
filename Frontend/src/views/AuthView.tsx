...
interface AuthViewProps {
  onSuccessfulAuth: (role: 'student' | 'instructor', token: string, userId: string) => void;
}
...
export default function AuthView({ onSuccessfulAuth }: AuthViewProps) {
  ...
  const handleLoginClick = () => {
    ...
    window.location.href = `https://${APP_NAME}.hub.loginradius.com/auth.aspx?action=login&return_url=${REDIRECT_URL}`;
  };
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      // (We pass 'student' and 'pending-id' as temporary mocks; the Go backend will extract the REAL role and ID from the token)
      onSuccessfulAuth('student', token, 'pending-id'); 
    }
  }, [onSuccessfulAuth]);

  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-950 p-6 font-sans text-gray-100">
      <div className="text-center w-full max-w-md p-8 rounded-2xl border border-gray-800 bg-gray-900/60 shadow-2xl backdrop-blur-md">
        ...
        <p className="text-gray-400 mb-8 text-sm">Authenticate securely via AuthStudio to access your dashboard.</p>
        
        <button 
          onClick={handleLoginClick} 
          className="w-full rounded-lg bg-blue-600 py-3 text-sm font-bold text-white shadow-md transition-colors hover:bg-blue-500"
        >
